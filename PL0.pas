...
type
  ...
  obj = (constant, variable, myProcedure);
...
procedure error (n : integer); begin
...
procedure getsym;
  ...
  begin {getsym}
    while ch = ' ' do getch;
    if ch in ['A'..'Z'] then begin {标识符或保留字} 
      k := 0;
      repeat 
        if k < al then begin 
          k:= k + 1; 
      ...
      if i-1 > j then 
        ...
        sym := ident;
    end else if ch in ['0'..'9'] then begin {数字} 
...
procedure gen(x : fct; y, z : integer); begin
...
procedure test(s1, s2 : symset; n : integer); begin
...
procedure block(lev, tx : integer; fsys : symset);
  ...
  procedure enter(k : obj); begin {把obj填入符号表中}
    tx := tx +1;
  ...
  procedure statement(fsys : symset);
    ...
    procedure readdata(fsys:symset); begin
      getsym;
      expression(fsys);
      i := position(id);
      if i = 0 then 
    ...
    procedure writedata(fsys:symset);begin
      getsym;
      expression(fsys);
      i := position(id);
      if i = 0 then 
    ...
    begin {statement}
      if sym = ident then begin 
        ...
        if i = 0 then 
          error(11) 
        else if table[i].kind <> variable then begin {对非变量赋值} 
          error(12); i := 0; 
        end;
        getsym;
        if sym = becomes then 
